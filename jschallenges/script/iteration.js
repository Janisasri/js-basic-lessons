...
const toppings = ['Mushrooms ', 'Tomatoes', 'Eggs', 'Chili', 'Lettuce', 'Avocado', 'Chiles', 'Bacon', 'Pickles', 'Onions', 'Cheese'];
  ...
  const orderTotals1 = [342, 1002, 523, 34, 634, 854, 1644, 2222];
  function increaseValue(item){
    return item * 1.25;
}
const newnumber=orderTotals1.map(increaseValue);
console.log(newnumber);

function tallyNumber(prvnumber,currentNumber) {
  console.log(`the total so far is ${prvnumber}`);
  console.log(`the current number is ${currentNumber}`);
  console.log('------------');
  //return the curret tally PLUS the amount of this order
  return prvnumber + currentNumber;

}
const allOrders = orderTotals.reduce(tallyNumber,0);
